...
const Slider = () => {
    const { banner } = useSelector((state) => state.homeData.home.data);
    console.log(banner);
    ...
    return (
        <div className="w-full px-5">
            <div className="flex w-full md:gap-8 sm:gap-2 pt-8">
                {banner?.items?.map((item, index) => (
                    <img
                        key={item.encodeId}
                        src={item.banner}
                        // onClick={() => handleClickBanner(item)}
                        className={`slider-item flex-1 object-contain w-1/5 rounded-lg }`}
                        alt="banner"
                    />
                ))}
            </div>
